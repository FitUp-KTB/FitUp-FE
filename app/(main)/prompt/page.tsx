  ...
  return (
    <div className="flex flex-row justify-center top-6 gap-8">
      <div className="min-h-screen flex flex-col items-center justify-start pt-36 relative">
        ...
        <Image src={Bot} alt="bot" width={128} height={128} className="absolute top-12 z-10" />
        ...
        <div className="bg-white rounded-[30px] shadow p-8 w-[500px] text-center relative">
          ...
          {!injuryStatus && (
            <>
              ...
              <div className="space-y-4">
                <Button variant="outline" className="w-full py-6 text-lg bg-BLUE text-WHITE rounded-full" onClick={() => handleInjurySelect("없음")}>
                  없음
                </Button>
                <Button variant="outline" className="w-full py-6 text-lg bg-BLUE text-WHITE rounded-full" onClick={() => handleInjurySelect("있음")}>
                  있음
                </Button>
              </div>
            </>
          ...
          {injuryStatus === "있음" && !selectedWorkout && !selectedSubWorkout && (
            <div className="mt-6">
              ...
              />
              <Button className="w-full py-3 text-lg mt-2 bg-BLUE text-WHITE rounded-full" onClick={handleSendRequest}>
                퀘스트 생성
              </Button>
            </div>
          )}
          ...
          {injuryStatus === "없음" && !selectedWorkout && (
            <div className="mt-6">
              ...
              <div className="space-y-4">
                {["유산소", "무산소", "기타"].map((type) => (
                  <Button key={type} variant="outline" className="w-full py-6 text-lg bg-BLUE text-WHITE rounded-full" onClick={() => handleWorkoutSelect(type)}>
                    {type}
                  </Button>
                ))}
              </div>
            ...
            <div className="mt-6">
              ...
              <div className="space-y-4">
                {subWorkouts[selectedWorkout].map((subType) => (
                  <Button key={subType} variant="outline" className="w-full py-6 text-lg bg-BLUE text-WHITE rounded-full" onClick={() => handleSubWorkoutSelect(subType)}>
                    {subType}
                  </Button>
                ))}
              </div>
          ...
          {selectedSubWorkout && (
            <div className="mt-6">
              ...
              />
              <Button className="w-full py-3 text-lg bg-BLUE text-WHITE rounded-full" onClick={handleSendRequest}>
                퀘스트 생성
              </Button>
            </div>
          )}
        ...
        <div className="bg-WHITE w-[400px] h-[500px] rounded-3xl shadow p-6 flex flex-col gap-4">
          ...
          <div className="gap-4 flex flex-col">
            ...
            <div className="flex flex-row justify-around">
              <Button variant="outline" onClick={handleSendRequest} className="border-BLACK w-2/5 bg-BLUE text-WHITE rounded-full">퀘스트 다시 만들기</Button>
              <Button onClick={handleQuestAccept} className="w-2/5 bg-BLUE text-WHITE rounded-full">퀘스트 수락하기</Button>
            </div>
